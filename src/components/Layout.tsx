  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50 dark:bg-slate-900 transition-colors duration-200">
      ...
      <aside className="hidden md:flex md:w-64 flex-col bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        {/* User profile card */}
        {user && (
          <div className={`p-4 mx-4 my-3 rounded-2xl flex items-center gap-3 border transition-all ${user.role === 'admin'
              ? 'bg-gradient-to-r from-amber-500/10 to-red-500/10 border-amber-500/30 dark:bg-amber-500/15'
              : 'bg-slate-50 dark:bg-slate-750 border-slate-200 dark:border-slate-700'
            }`}>
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black shrink-0 ${user.role === 'admin'
                ? 'bg-gradient-to-tr from-amber-500 to-red-500 text-white shadow-md shadow-amber-500/20'
                : 'bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200'
              }`}>
              {user.role === 'admin' ? <Crown className="w-5 h-5" /> : <UserCheck className="w-5 h-5 text-blue-500" />}
            </div>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-700 flex flex-col gap-2">
          ...
          >
            ...
            <span className="w-8 h-4 rounded-full bg-slate-200 dark:bg-slate-600 flex items-center p-0.5 transition-all">
              <span className={`w-3 h-3 rounded-full bg-white dark:bg-slate-800 shadow-sm transition-all transform ${config.theme === 'dark' ? 'translate-x-4' : 'translate-x-0'
                }`} />
            </span>
          </button>
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => setView(item.id)}
              className={`flex flex-col items-center gap-0.5 py-1 px-3 rounded-lg focus:outline-none transition-all cursor-pointer ${isActive
